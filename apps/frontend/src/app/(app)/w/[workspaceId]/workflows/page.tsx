...
export default function WorkflowsPage() {
  ...
  const loadWorkflows = async () => {
    try {
      ...
      // Load from SaaS endpoint
      const response = await apiClient.get<{ success: boolean; data: { workflows: WorkflowItem[] } }>(
        `/api/saas-workflows?workspaceId=${workspaceId}&includePublic=true`
      );
  ...
  const handleRun = async (workflowId: string) => {
    try {
      const response = await apiClient.post<{ success: boolean; data: { runId: string } }>(
        `/api/runs/${workflowId}/run`,
        { inputData: {} }
      );

      if (response.data.runId) {
        router.push(`/w/${workspaceId}/runs/${response.data.runId}`);
      }
    } catch (error: any) {
      console.error('Error running workflow:', error);
      alert(error.message || 'Failed to start workflow execution');
    }
  };
          ...
          {filteredWorkflows.map((workflow) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`px-2 py-1 rounded text-xs ${workflow.active
            ...
            <tbody className="divide-y divide-white/10">
              {filteredWorkflows.map((workflow) => (
                <tr key={workflow.id} className="hover:bg-white/5 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs ${workflow.active
                      ? 'bg-green-500/20 text-green-400'
                      : 'bg-gray-500/20 text-gray-400'
                      }`}>
                      {workflow.active ? 'Active' : 'Inactive'}
