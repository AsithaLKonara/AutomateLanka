...
export default function NewWorkflowPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Import workflow
      const response = await apiClient.post<any>('/api/workflows/import', {
        workspaceId,
    ...
    <div className="p-8">
      ...
      <div className="flex space-x-4 mb-8">
        <button
          ...
          className={`flex-1 p-6 rounded-xl border-2 transition ${method === 'upload'
              ? 'border-purple-500 bg-purple-500/20'
              : 'border-white/10 bg-white/5 hover:border-white/20'
            }`}
        >
          <Upload className={`w-8 h-8 mx-auto mb-2 ${method === 'upload' ? 'text-purple-400' : 'text-white/50'
            }`} />
          <div className={`font-semibold ${method === 'upload' ? 'text-white' : 'text-white/70'
            }`}>
            Upload JSON
          </div>
        ...
        <button
          ...
          className={`flex-1 p-6 rounded-xl border-2 transition ${method === 'create'
              ? 'border-purple-500 bg-purple-500/20'
              : 'border-white/10 bg-white/5 hover:border-white/20'
            }`}
          disabled
        >
          <Plus className={`w-8 h-8 mx-auto mb-2 ${method === 'create' ? 'text-purple-400' : 'text-white/50'
            }`} />
          <div className={`font-semibold ${method === 'create' ? 'text-white' : 'text-white/70'
            }`}>
            Create New
          </div>
          <div className="text-white/50 text-sm mt-1">
